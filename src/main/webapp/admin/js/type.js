        ...
        columns:[[
            ...
            {field:'option',title:'操作',width:100,
                formatter: function(value,row,index){
                    return "<a href='javascript:deleteType("+row.id+")'>删除</a>|<a href='javascript:updateDetail("+row.id+")'>编辑</a>"
                }
            }
        ]]
    });
...
function updateDetail(id) {
    ...
    $.post("listType",{id:id},function (data) {
...
function ModifyBySelect() {
    ...
    if(selections.length==1){
        $("#updateDialog").dialog("open").dialog('setTitle',"编辑区域");
        // $("#AddDialog").form("load",selections[0]);
        $.post("listType",{id:selections[0].id},function (data) {
            $("#updateDialog").form("load",data);
        },"json")
    }else{
