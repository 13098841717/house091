        ...
        columns:[[
            ...
            {field:'option',title:'操作',width:100,
                formatter: function(value,row,index){
                    return "<a href='javascript:deleteUsers("+row.id+")'>删除</a>|<a href='javascript:updateDetail("+row.id+")'>编辑</a>"
                }
            }
        ]]
    });
    ...
    if(selections.length>0){
        ...
        $.messager.confirm("系统提示", "是否确认删除",function (r) {
            if(r){
                ...
                //测试
                // alert(str);
                $.post("deleteMoreUsers",{ids:str},function (data) {
...
function updateDetail(id) {
    ...
    $.post("listUsers",{id:id},function (data) {
...
function ModifyBySelect() {
    ...
    if(selections.length==1){
        $("#updateDialog").dialog("open").dialog('setTitle',"编辑区域");
        // $("#AddDialog").form("load",selections[0]);
        $.post("listUsers",{id:selections[0].id},function (data) {
            $("#updateDialog").form("load",data);
        },"json")
    }else{
