...
<TABLE class=field>
  <TBODY>
  <TR>
    <TD colSpan=2><span id="info" style="color: red"></span></TD></TR>
  <TR>
  ...
  <TR>
    ...
    <TD><!-- <input type="password" class="text" name="password" /> --><INPUT 
      id=user_password class=text type=password name=password> </TD></TR><!--
						...
						--></TBODY></TABLE>
<DIV class=buttons> <INPUT  value=登陆 id="bn" name="sum1" type="button"><INPUT onclick='document.location="regs.jsp"' value=注册 type=button>
...
<script>
  $(function () {
      $("#bn").click(function () {
          $.post("login",{"username":$("#user_name").val(),"password":$("#user_password").val()},function (data) {
              if(data.info=='用户名错误'||data.info=='密码错误'){
                $("#info").html(data.info);
              }else{
                window.location.href="houseData";
              }
          },"json")
      })
  })
